...
NewGVN::performSymbolicLoadCoercion(Type *LoadType, Value *LoadPtr,
  ...
  if (auto *DepSI = dyn_cast<StoreInst>(DepInst)) {
    ...
    // Don't need to coerce if they are the same type, we will just propgate..
    ...
    if (Offset >= 0) {
      if (auto *C = dyn_cast<Constant>(
        ...
        DEBUG(dbgs() << "Coercing load from store " << *LI << " to constant "
                     << *C << "\n");
    ...
    if (Offset >= 0) {
      if (auto *PossibleConstant =
        ...
        DEBUG(dbgs() << "Coercing load from meminst " << *LI << " to constant "
                     << *PossibleConstant << "\n");
        return createConstantExpression(PossibleConstant);
      }
    }
  ...
  // All of the below are only true if the loaded pointer and the dependent
  // pointer are the same
  if (lookupOperandLeader(LoadPtr) != lookupOperandLeader(DepInst) &&
  ...
  if (!MSSA->isLiveOnEntryDef(DefiningAccess)) {
    if (auto *MD = dyn_cast<MemoryDef>(DefiningAccess)) {
      ...
      if (!ReachableBlocks.count(DefiningInst->getParent()))
        return createConstantExpression(UndefValue::get(LI->getType()));
      // This will handle stores and memory insts.
      // We only do if it the defining access has a different type.
      const Expression *CoercionResult =
        performSymbolicLoadCoercion(LI->getType(), LoadAddressLeader,
                                      LI, DefiningInst, DefiningAccess);
      if (CoercionResult)
